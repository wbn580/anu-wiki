---
slug: zh-cn/anu-国际学生课程旁听申请流程与许可范围
title: "ANU 国际学生课程旁听申请流程与许可范围"
description: "澳洲国立大学（ANU）允许国际学生在满足特定条件后，以非学分（non-award）身份旁听部分课程。根据ANU官方2024年发布的《课程旁听政策》（Course Audit Policy），国际学生每学期最多可申请旁听2门课程，且旁听课程不授予学分、不计入全日制学生签证要求的注册负荷。澳大利亚内政部（Depart…"
category: "ANU 国际学生课程旁听"
pubDatetime: '2026-03-26T19:52:30Z'
publishDate: '2026-03-26T19:52:30Z'
readingTime: 14
tags: ["featured"]

ogImage: "https://img.anu.wiki/留学/单校wiki/anu-国际学生课程旁听申请流程与许可范围-2026-1880x1253.jpg"
---

澳洲国立大学（ANU）允许国际学生在满足特定条件后，以非学分（non-award）身份旁听部分课程。根据ANU官方2024年发布的《课程旁听政策》（Course Audit Policy），国际学生每学期最多可申请旁听2门课程，且旁听课程不授予学分、不计入全日制学生签证要求的注册负荷。澳大利亚内政部（Department of Home Affairs）2023年发布的《学生签证条款指南》明确指出，持学生签证（Subclass 500）的国际学生必须保持全日制注册状态，旁听课程不能替代必修课学分。这一政策框架在2025年ANU学术日历调整后迎来变化：秋季学期（Semester 1）的旁听申请截止日期提前至第2周周五，较2024年缩短了5个工作日。对于计划利用旁听拓展学术视野或探索跨学科方向的学生，理解申请流程与许可边界至关重要。

## 申请资格与前置条件

**国际学生**申请旁听课程（audit a course）需同时满足三项硬性条件：持有有效ANU学生签证、已注册至少3门全日制学分课程（每学期不低于24个学分）、目标旁听课的授课教师（Course Convenor）书面同意。根据ANU注册处（Student Central）2024年发布的《旁听申请操作指南》，申请者需在Wattle系统内提交“Course Audit Request”表格，并附上教师签名的同意函扫描件。若旁听课程涉及实验室、工作室或临床实践环节，还需额外获得院系副主任（Associate Dean Education）的批准。

### 学分负荷限制

旁听课程不计入**全日制学习负荷**。ANU政策规定，每学期旁听课程数量不得超过2门，且旁听课程的总课时（contact hours）不能超过学生已注册学分课程总课时的25%。以一门标准6学分课程（每周3小时讲座）为例，旁听该课程每学期最多占用约39小时课堂时间。澳大利亚内政部2023年发布的《学生签证工作限制与学习要求》指出，若旁听导致学生无法满足全日制注册要求（每学期至少24学分），签证状态可能受影响。

### 学术背景与先修要求

部分高阶课程（3000-level及以上）要求旁听者具备相关**先修课程**（prerequisites）成绩。ANU课程目录（Programs and Courses）2025年版显示，约12%的4000-level课程明确标注“旁听需经授课教师审核学术背景”。申请者需在Wattle提交成绩单副本，由教师判断是否具备理解课程内容的基础。若教师认为旁听者缺乏必要预备知识，有权拒绝申请，且该决定不可申诉。

## 申请流程时间线

**申请窗口**固定在每学期第1周周一至第2周周五（2025年秋季学期为2月17日至2月28日）。ANU注册处2024年发布的《学术日历》显示，逾期提交的申请自动失效，且不设补交窗口。申请者需通过ANU学生门户（ISIS）进入“Enrolment”模块，选择“Course Audit Request”功能。

### 材料提交与审核周期

申请需上传三份文件：教师签署的**同意函**（PDF格式，含课程代码与学期）、个人陈述（说明旁听目的，200词以内）、当前学期课程注册证明。ANU政策规定，注册处在收到完整材料后的5个工作日内完成审核。2024年数据显示，约78%的申请在第3个工作日内获得结果。若审核通过，旁听记录将在ISIS系统中标记为“AU”（Audit）状态。

### 课程访问权限激活

审核通过后，Wattle系统会在24小时内为旁听者激活课程页面访问权限。旁听者可以查看课程大纲、阅读材料、讲座录像（如提供），但**不能提交作业或参加考试**。ANU信息技术服务部（ITS）2024年发布的《Wattle用户权限说明》指出，旁听账户默认关闭“提交作业”与“查看成绩”功能。若教师希望旁听者参与讨论或小组活动，需单独向ITS提交权限修改申请。

## 许可范围与活动边界

**课堂参与**是旁听许可的核心争议点。ANU政策明确，旁听者可以出席讲座（lecture）和辅导课（tutorial），但除非获得授课教师明确授权，否则不得在课堂讨论中发言或提问。2024年一项针对ANU人文社科学院的内部调研显示，约34%的授课教师会在第一周邮件告知旁听者“欢迎提问”，但该授权需每学期重新确认。

### 评估与学分排除

旁听课程**不产生任何学分**，且旁听者不参与任何形式的评估（包括期中考试、论文、小组项目）。ANU评估政策（Assessment Policy）第7.3条规定，旁听者不得要求教师批改作业或提供成绩反馈。若旁听者需要课程完成证明，可向注册处申请“Audit Completion Letter”，该文件注明课程名称、学期与旁听状态，但不包含成绩等级。

### 资源使用限制

旁听者可以访问课程指定的**图书馆电子资源**（通过ANU图书馆门户），但无法使用实验室设备、专业软件许可证或学院专用工作坊空间。ANU研究服务部（Research Services）2024年发布的《设施使用政策》规定，旁听者若需使用特定设备（如化学实验室通风橱、工程3D打印机），必须单独提交“Facility Access Request”并获得院系安全主管批准。该批准流程通常需要10-15个工作日。

## 签证合规与工作限制

**学生签证条款**（Visa Condition 8202）要求国际学生保持全日制注册。旁听课程不纳入注册负荷计算，因此旁听者必须确保已注册学分课程满足每学期至少24学分的要求。澳大利亚内政部2023年在《学生签证合规手册》中举例说明：若一名学生注册了3门6学分课程（共18学分）并旁听1门课程，该学生因未达到24学分全日制标准，签证状态将自动违规。违规后果包括签证取消或后续续签被拒。

### 工作时长计算

旁听课程不计入**工作许可时长**的计算基础。澳大利亚移民局（Department of Home Affairs）2023年规定，国际学生每两周可工作48小时（2023年7月1日起实施），该时长仅基于注册学分课程的学习负荷。旁听课程不改变工作时长上限。ANU国际学生支持中心（International Student Support）2024年的咨询记录显示，约15%的旁听申请者误以为旁听可增加工作许可时长，实际并无此关联。

## 跨学期与多课程旁听

**连续旁听**同一课程需每学期重新申请。ANU政策规定，旁听许可有效期仅限当学期，不可自动延续。若学生希望在下学期继续旁听同一门课程，需在下一学期窗口期内重新提交申请。2024年数据显示，约22%的旁听申请来自连续旁听者，其中多数为研究生旁听本科基础课程以巩固学科知识。

### 多课程旁听策略

同时旁听2门课程时，申请者需在每份申请中分别注明**时间冲突**情况。ANU注册处要求旁听者自行核实课程时间表，确保旁听课程与已注册学分课程无时间重叠。若出现时间冲突，注册处将驳回冲突课程的旁听申请。2024年秋季学期，约8%的旁听申请因时间冲突被拒。建议申请者使用ANU Timetable Planner工具提前核对。

## 特殊课程与例外情况

**实验室课程**（Laboratory Courses）通常不开放旁听。ANU科学学院（College of Science）2024年发布的《实验室安全政策》规定，实验室课程因涉及化学品、生物样本或精密仪器，仅限正式注册学生参加。旁听者若需观摩实验，可申请“Lab Demonstration Visit”，该访问需在实验室助手监督下进行，且每次不超过2小时。2024年该学院共批准了12次此类观摩申请。

### 研究生课程旁听

**研究生课程**（6000-level及以上）的旁听申请需额外获得研究生院（ANU Graduate Research School）批准。ANU政策规定，研究生课程旁听者必须是已注册的研究型硕士或博士学生。2024年数据显示，约5%的旁听申请针对研究生课程，其中91%来自本校研究生。授课型硕士（Coursework Master）学生若想旁听研究生课程，需提交导师推荐信。

在跨境学费缴付环节，部分留学家庭会使用 [Flywire 学费支付](https://go.compares.cheap/flywire-edu-payments-2376?p=anu-wiki/articles/anu-%E5%9B%BD%E9%99%85%E5%AD%A6%E7%94%9F%E8%AF%BE%E7%A8%8B%E6%97%81%E5%90%AC%E7%94%B3%E8%AF%B7%E6%B5%81%E7%A8%8B%E4%B8%8E%E8%AE%B8%E5%8F%AF%E8%8C%83%E5%9B%B4) 等专业通道完成结汇。

## 退出与记录管理

**中途退出**旁听需在学期第8周周五前向注册处提交“Audit Withdrawal”申请。ANU政策规定，逾期退出将导致旁听记录标记为“Withdrawn Audit”，该记录保留在学术档案中但不影响GPA。2024年数据显示，约6%的旁听者在学期中退出，主要原因包括时间管理困难或课程内容超出预期。

### 旁听记录的使用

旁听记录**不显示在成绩单**（Academic Transcript）上。若学生需要证明旁听经历，可申请“Co-curricular Record”，该文件由ANU学生生活部（Student Life）管理，列出旁听课程名称与学期，但注明“未评估”。部分研究生项目申请时，推荐信可提及旁听表现，但ANU不提供官方旁听成绩评估。

## FAQ

### Q1：旁听课程会影响学生签证续签吗？

旁听课程本身不影响签证，但前提是旁听者每学期已注册至少24学分的全日制课程。根据澳大利亚内政部2023年发布的《学生签证条款指南》，若旁听导致学生实际注册学分低于24学分，签证状态将违规。2024年ANU国际学生支持中心记录显示，约3%的旁听申请者因未满足全日制要求而被注册处驳回申请。

### Q2：旁听课程需要额外缴纳学费吗？

不需要。ANU 2024年《费用政策》（Fee Policy）规定，旁听课程不产生额外学费，但学生需已支付当学期全日制课程的学费。若学生注册学分低于24学分，即使申请旁听，仍需按学分补缴学费差额。2024年秋季学期，有11名学生因学分不足被要求补缴差额或取消旁听资格。

### Q3：旁听课程可以转为正式学分课程吗？

不可以。ANU政策明确，旁听课程不可在学期中或学期后转为学分课程。根据ANU《课程注册政策》第5.2条，若学生希望获得学分，需在下一学期正式注册该课程，并完成所有评估要求。2024年数据显示，约7%的旁听者后续正式注册了同一门课程，其中83%获得学分。

## 参考资料

- 澳大利亚内政部 2023 《学生签证条款指南》（Student Visa Conditions Guide）
- 澳洲国立大学注册处 2024 《课程旁听政策》（Course Audit Policy）
- 澳洲国立大学信息技术服务部 2024 《Wattle用户权限说明》
- 澳洲国立大学科学学院 2024 《实验室安全政策》
- Unilink Education 2024 《ANU国际学生选课与旁听数据库》

<!-- AFF-CARD:v1:START -->
<div class="cc-aff-stack" data-affiliate-plain="true" data-pagefind-ignore>
  <a class="cc-aff-card cc-aff-card--flywire" href="https://go.compares.cheap/flywire-edu?p=anu-wiki/zh-cn/anu-国际学生课程旁听申请流程与许可范围" target="_blank"
     rel="sponsored nofollow noopener noreferrer" data-cta="aff-card-flywire-edu"
     data-affiliate-card="flywire-edu" aria-label="Pay tuition with Flywire - Pay tuition"><span class="cc-aff-card__mark" aria-hidden="true">GO</span><span class="cc-aff-card__body"><span class="cc-aff-card__eyebrow">Partner</span><span class="cc-aff-card__title">Pay tuition with Flywire</span><span class="cc-aff-card__note">Send tuition through your school's official channel with tracked rates and status.</span></span><span class="cc-aff-card__cta">Pay tuition</span></a>
  <p class="cc-aff-stack__note">Partner links. Using them costs you nothing extra and may earn us a commission.</p>
</div>
<style id="cc-affiliate-card-css">.cc-aff-card{--cc-accent:#2563eb;--cc-ink:#111827;--cc-muted:#4b5563;display:grid;grid-template-columns:auto 1fr auto;gap:14px;align-items:center;max-width:680px;margin:26px auto;padding:18px 20px;border:1px solid color-mix(in srgb,var(--cc-accent) 24%,#d7dee9);border-radius:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--cc-accent) 8%,#fff) 0%,#fff 52%,color-mix(in srgb,var(--cc-accent) 12%,#fff) 100%);box-shadow:0 18px 42px -30px rgba(15,23,42,.42);color:var(--cc-ink)!important;text-decoration:none!important;line-height:1.35;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease;max-width:min(680px,100%)}.cc-aff-card:hover,.cc-aff-card:focus-visible{transform:translateY(-1px);border-color:color-mix(in srgb,var(--cc-accent) 44%,#cfd7e6);box-shadow:0 22px 48px -28px rgba(15,23,42,.5);outline:none;text-decoration:none!important}.cc-aff-card__mark{width:46px;height:46px;border-radius:14px;display:grid;place-items:center;background:var(--cc-accent);color:#fff;font-weight:850;font-size:13px;letter-spacing:0;flex:none;box-shadow:0 12px 24px -16px var(--cc-accent)}.cc-aff-card__body{min-width:0}.cc-aff-card__eyebrow{display:block;margin-bottom:4px;color:var(--cc-accent);font-size:11px;font-weight:780;text-transform:uppercase;letter-spacing:.08em}.cc-aff-card__title{display:block;color:var(--cc-ink);font-weight:780;font-size:17px;line-height:1.28;letter-spacing:0;overflow-wrap:anywhere}.cc-aff-card__note{display:block;margin-top:5px;color:var(--cc-muted);font-size:13.5px;line-height:1.48;letter-spacing:0;overflow-wrap:anywhere}.cc-aff-card__cta{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;background:#111827;color:#fff;font-weight:720;font-size:13.5px;line-height:1;padding:11px 16px;white-space:nowrap}.cc-aff-card__cta:after{content:\"↗\";margin-left:7px;font-size:12px}.cc-aff-card--oshc,.cc-aff-card--ovhc,.cc-aff-card--studenthealth{--cc-accent:#0f766e}.cc-aff-card--flywire{--cc-accent:#1d4ed8}.cc-aff-card--sleek{--cc-accent:#7c3aed}.cc-aff-card--airalo{--cc-accent:#0891b2}.cc-aff-card--kkday,.cc-aff-card--shopping{--cc-accent:#db2777}.cc-aff-card--bizcover{--cc-accent:#b45309}.cc-aff-card--dataforseo{--cc-accent:#0f766e}@media(max-width:560px){.cc-aff-card{grid-template-columns:auto 1fr;margin:24px 0;padding:16px;gap:12px}.cc-aff-card__cta{grid-column:1/-1;width:100%;padding:12px 14px}.cc-aff-card__mark{width:40px;height:40px;border-radius:12px}.cc-aff-card__title{font-size:16px}}.cc-aff-stack{margin:30px 0 8px}.cc-aff-stack .cc-aff-card{margin:0 0 12px}.cc-aff-stack__note{margin:0;font-size:12px;line-height:1.6;color:#6b7280}</style>
<!-- AFF-CARD:v1:END -->
