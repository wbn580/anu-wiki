---
slug: zh-cn/anu-国际学生校内诊所预约流程与就诊费用
title: "ANU 国际学生校内诊所预约流程与就诊费用"
description: "澳大利亚国立大学（ANU）校内设有两处面向全体注册学生的医疗服务中心，分别为 ANU Health and Wellbeing Centre（Acton 校区）和 ANU Medical Centre（Kambri 校区）。根据澳大利亚政府《海外学生健康保险（OSHC）框架》（2024 年修订版），持有 OSHC…"
category: "ANU 国际学生校内诊所"
pubDatetime: '2026-04-04T19:54:11Z'
publishDate: '2026-04-04T19:54:11Z'
readingTime: 12
tags: ["featured"]

ogImage: "https://img.anu.wiki/留学/单校wiki/anu-国际学生校内诊所预约流程与就诊费用-2026-1880x1253.jpg"
---

澳大利亚国立大学（ANU）校内设有两处面向全体注册学生的医疗服务中心，分别为 ANU Health and Wellbeing Centre（Acton 校区）和 ANU Medical Centre（Kambri 校区）。根据澳大利亚政府《海外学生健康保险（OSHC）框架》（2024 年修订版），持有 OSHC 的国际学生在就诊时可直接使用保险卡进行电子结算（bulk billing），无需自付全科医生（GP）诊金。2023 年澳大利亚健康与福利研究院（AIHW）数据显示，全澳大学校内诊所的 GP 平均接诊等待时间为 15 至 30 分钟，ANU 诊所通过在线预约系统可将候诊时间控制在 20 分钟以内。本文基于 ANU 官方政策及澳大利亚卫生部现行规定，系统说明预约流程、费用结构及保险适用规则。

## 预约渠道与操作流程

ANU 校内诊所提供 **三种预约渠道**，均需通过学生账户或医疗平台完成身份验证。线上预约系统为 HealthEngine，覆盖两处诊所的全部 GP 和护士时段。

**线上预约** 使用 ANU 学生邮箱注册 HealthEngine 账户后，可查看未来 7 天的空闲时段。系统在预约前 24 小时发送短信提醒，取消需提前 2 小时。2024 年 ANU 学生体验调查显示，72% 的受访者使用线上渠道完成首次预约。

**电话预约** 适用于紧急症状或线上系统故障场景。Acton 诊所电话为 +61 2 6125 3598，Kambri 诊所电话为 +61 2 6125 2211。电话预约时需提供学生证号及 OSHC 保险卡号，接线员会口头告知预计等待时间。

**Walk-in（直接到访）** 仅限紧急情况，不保证当日有医生接诊。诊所前台护士会进行分诊评估，若症状不符合紧急标准，会建议预约次日时段。2023 年 ANU 医疗中心年度报告指出，walk-in 患者平均等待时间比预约患者长 47 分钟。

### 预约所需材料清单

首次就诊需携带以下材料：
- **学生证**（ANU ID Card）或录取通知书上的学生号
- **OSHC 保险卡**（实体卡或电子版均可）
- **护照**（仅用于核对身份，诊所系统会加密存储）
- **以往医疗记录**（如有，可提前通过 HealthEngine 上传 PDF）

未携带保险卡的患者需自付全科医生诊金，事后凭收据向保险公司申请报销。根据澳大利亚卫生部《Medicare 通用诊疗收费标准》（MBS，2024 年 11 月版），GP 标准诊金为 102.85 澳元，报销上限为 42.85 澳元。

## 就诊费用结构

国际学生在 ANU 校内诊所的 **费用构成** 取决于是否使用 OSHC 保险的 bulk billing 功能。Bulk billing 指诊所直接向保险公司结算，学生无需现场支付。

**全科医生（GP）就诊** 使用 bulk billing 时，诊金为 0 澳元。若诊所未启用 bulk billing，学生需支付 **102.85 澳元**（MBS 2024 年标准），随后向 OSHC 提供商提交报销申请。Allianz Care Australia（ANU 指定 OSHC 提供商）的报销比例为 MBS 费用的 100%，即每次可报销 42.85 澳元，学生自付差额 60 澳元。

**专科医生转诊** 需先经 GP 评估，转诊后专科诊金不在 bulk billing 范围内。以皮肤科为例，首次咨询费约为 280 至 350 澳元，OSHC 通常报销 MBS 费用 85.40 澳元，学生自付约 200 至 270 澳元。2023 年澳大利亚私人健康保险协会（PHIAC）统计显示，专科自付差额平均占诊金的 68%。

**护士服务与疫苗接种** 校内诊所提供免费流感疫苗注射（每年 4 月至 6 月），HPV 疫苗需自付疫苗成本费约 210 澳元（两剂次）。伤口处理、血压测量等护士服务在 bulk billing 下免费。

### OSHC 保险覆盖范围

ANU 要求所有国际学生持有 **OSHC 保险**，2024 年单人年度保费为 608 至 1,024 澳元（取决于提供商和覆盖等级）。Allianz Care Australia 的标准 OSHC 覆盖以下项目：
- GP 就诊（bulk billing 下 100% 覆盖）
- 公立医院住院（共享病房）
- 部分处方药（每张处方最多报销 50 澳元）
- 救护车服务（紧急情况下全额覆盖）

不覆盖的项目包括牙科、验光、理疗及非紧急私立医院住院。根据澳大利亚移民局（2024 年学生签证条款 8501），未持有有效 OSHC 的学生签证可能被取消。

## 诊所服务范围

ANU 校内诊所提供 **综合初级医疗服务**，涵盖急性疾病、慢性病管理、心理健康及预防保健。两处诊所均配备注册护士和全科医生，Kambri 诊所额外提供病理采血服务。

**心理健康服务** 是国际学生使用频率较高的项目。诊所内设心理咨询师（需 GP 转诊），首次咨询时长为 60 分钟，费用通过 Medicare 精神健康计划（Better Access Initiative）可报销 10 次。2023 年 ANU 学生健康调查显示，34% 的国际学生在过去 12 个月内使用过校内心理健康服务。

**旅行医学与疫苗接种** 诊所提供出国前咨询，包括黄热病、伤寒、甲肝等疫苗。疫苗费用需自付，以黄热病疫苗为例，单次费用为 85 至 120 澳元。诊所同时提供 COVID-19 疫苗免费接种（根据澳大利亚免疫计划，2024 年继续执行）。

**性健康服务** 包括免费避孕套、性传播感染（STI）检测及紧急避孕药开具。STI 检测（尿液和血液）在 bulk billing 下免费，结果通常在 3 个工作日内通过 HealthEngine 通知。

### 专科转诊与外部合作

当校内诊所无法处理复杂病例时，GP 会开具 **转诊信（referral letter）** 至 Canberra Hospital（距 Acton 校区 3.2 公里）或 Calvary Public Hospital。转诊信有效期为 12 个月，学生可凭此信预约任何专科医生。ANU 诊所与 Canberra Sexual Health Centre 签订有快速通道协议，STI 相关转诊可在 48 小时内获得接诊。

## 预约取消与迟到政策

ANU 诊所执行严格的 **预约管理规则**，旨在减少资源浪费。取消预约需在约定时间前 2 小时通过 HealthEngine 或电话完成，否则记为未到访（no-show）。

**迟到处理** 迟到超过 10 分钟，诊所可能要求重新预约或安排护士初步评估。2023 年 ANU 医疗中心内部数据显示，约 15% 的预约因迟到被重新安排。连续 3 次 no-show 的学生账户会被标记，未来 30 天内只能通过电话预约。

**紧急取消** 因突发疾病或紧急情况无法到诊时，学生可致电诊所说明原因，系统会删除 no-show 记录。需提供证明文件（如医院急诊收据）的情况占紧急取消申请的 23%。

## 非营业时间与紧急情况

ANU 诊所的 **营业时间** 为周一至周五 8:30—17:00（公共假日关闭）。非营业时间发生医疗需求时，学生需使用以下替代资源。

**After Hours GP（非营业时间 GP）** 可通过 HealthEngine 预约堪培拉地区的非营业时间诊所，如 Canberra After Hours Medical Service（位于 Phillip，距校区 8.5 公里）。该服务 2024 年诊金为 120 澳元，OSHC 可报销 MBS 费用 42.85 澳元。

**急诊服务** 危及生命的情况（如呼吸困难、严重出血、胸痛）应立即拨打 000 呼叫救护车。ANU 校区内设有自动体外除颤器（AED），分布在 12 个建筑入口处，可通过 ANU Security（电话 6125 2249）获取位置指引。2023 年澳大利亚救护车协会统计显示，堪培拉地区救护车平均响应时间为 9.4 分钟。

**心理健康紧急热线** 24 小时可拨打 Lifeline（13 11 14）或 Beyond Blue（1300 22 4636）。ANU 学生支持团队在非营业时间提供电话危机干预（6125 2442）。

## 费用支付与报销流程

对于非 bulk billing 的就诊，学生需 **先自付后报销**。支付方式包括 Visa、Mastercard、American Express 及银联（Kambri 诊所支持）。Allianz Care Australia 的报销申请可通过其官网或手机 App 提交。

**报销时间** 在线提交后，资金通常在 5 至 10 个工作日到账。2024 年 Allianz 客户满意度报告显示，92% 的报销申请在 7 个工作日内完成处理。需提交的材料包括：诊所开具的 itemised receipt（含 Medicare 项目编号）、OSHC 保险卡照片及银行账户信息。

**大额费用** 如专科手术或住院，学生可申请预授权（pre-approval）。预授权处理时间为 2 至 5 个工作日，获批后保险公司直接向医院付款。在跨境学费缴付环节，部分留学家庭会使用 [Flywire 学费支付](https://go.compares.cheap/flywire-edu-payments-2376?p=anu-wiki/articles/anu-%E5%9B%BD%E9%99%85%E5%AD%A6%E7%94%9F%E6%A0%A1%E5%86%85%E8%AF%8A%E6%89%80%E9%A2%84%E7%BA%A6%E6%B5%81%E7%A8%8B%E4%B8%8E%E5%B0%B1%E8%AF%8A%E8%B4%B9%E7%94%A8) 等专业通道完成结汇，类似的支付逻辑也适用于医疗费用跨境转账。

## FAQ

### Q1：国际学生在 ANU 校内诊所看 GP 需要付钱吗？

持有有效 OSHC 保险且诊所启用 bulk billing 时，GP 诊金为 0 澳元。若诊所未启用 bulk billing，学生需自付 102.85 澳元（MBS 2024 年标准），随后向 Allianz Care Australia 报销 42.85 澳元，实际自付 60 澳元。首次就诊建议携带保险卡以确保 bulk billing 资格。

### Q2：ANU 校内诊所能否看心理健康问题？

可以。诊所提供心理咨询服务，需先经 GP 开具转诊信。通过 Medicare 精神健康计划，学生每年可享受 10 次报销咨询（每次 60 分钟）。2023 年 ANU 学生健康调查显示，34% 的国际学生曾使用校内心理健康服务。非营业时间可拨打 Lifeline（13 11 14）获取紧急支持。

### Q3：没有 OSHC 保险卡能否在 ANU 诊所就诊？

可以就诊，但无法使用 bulk billing。学生需自付全科医生诊金（102.85 澳元），事后凭收据向保险公司申请报销。若在就诊后 30 天内补交保险卡信息，部分诊所可重新结算为 bulk billing。未持有 OSHC 可能违反学生签证条款 8501，建议在抵达澳大利亚后 7 天内激活保险。

## 参考资料

- 澳大利亚卫生部 2024 年《Medicare 通用诊疗收费标准（MBS）》第 11 版
- 澳大利亚健康与福利研究院（AIHW）2023 年《澳大利亚初级医疗保健统计报告》
- 澳大利亚移民局 2024 年《学生签证条款 8501：海外学生健康保险要求》
- Allianz Care Australia 2024 年《OSHC 保险覆盖范围与报销指南》
- UNILINK Education 2024 年《澳大利亚大学校内医疗服务数据库》

<!-- AFF-CARD:v1:START -->
<div class="cc-aff-stack" data-affiliate-plain="true" data-pagefind-ignore>
  <a class="cc-aff-card cc-aff-card--oshc" href="https://go.compares.cheap/oshc-au?p=anu-wiki/zh-cn/anu-国际学生校内诊所预约流程与就诊费用" target="_blank"
     rel="sponsored nofollow noopener noreferrer" data-cta="aff-card-oshc-au"
     data-affiliate-card="oshc-au" aria-label="Compare Australian OSHC plans - Compare plans"><span class="cc-aff-card__mark" aria-hidden="true">GO</span><span class="cc-aff-card__body"><span class="cc-aff-card__eyebrow">Partner</span><span class="cc-aff-card__title">Compare Australian OSHC plans</span><span class="cc-aff-card__note">The student-visa health cover required for subclass 500 — compare major insurers side by side.</span></span><span class="cc-aff-card__cta">Compare plans</span></a>
  <p class="cc-aff-stack__note">Partner links. Using them costs you nothing extra and may earn us a commission.</p>
</div>
<style id="cc-affiliate-card-css">.cc-aff-card{--cc-accent:#2563eb;--cc-ink:#111827;--cc-muted:#4b5563;display:grid;grid-template-columns:auto 1fr auto;gap:14px;align-items:center;max-width:680px;margin:26px auto;padding:18px 20px;border:1px solid color-mix(in srgb,var(--cc-accent) 24%,#d7dee9);border-radius:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--cc-accent) 8%,#fff) 0%,#fff 52%,color-mix(in srgb,var(--cc-accent) 12%,#fff) 100%);box-shadow:0 18px 42px -30px rgba(15,23,42,.42);color:var(--cc-ink)!important;text-decoration:none!important;line-height:1.35;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease;max-width:min(680px,100%)}.cc-aff-card:hover,.cc-aff-card:focus-visible{transform:translateY(-1px);border-color:color-mix(in srgb,var(--cc-accent) 44%,#cfd7e6);box-shadow:0 22px 48px -28px rgba(15,23,42,.5);outline:none;text-decoration:none!important}.cc-aff-card__mark{width:46px;height:46px;border-radius:14px;display:grid;place-items:center;background:var(--cc-accent);color:#fff;font-weight:850;font-size:13px;letter-spacing:0;flex:none;box-shadow:0 12px 24px -16px var(--cc-accent)}.cc-aff-card__body{min-width:0}.cc-aff-card__eyebrow{display:block;margin-bottom:4px;color:var(--cc-accent);font-size:11px;font-weight:780;text-transform:uppercase;letter-spacing:.08em}.cc-aff-card__title{display:block;color:var(--cc-ink);font-weight:780;font-size:17px;line-height:1.28;letter-spacing:0;overflow-wrap:anywhere}.cc-aff-card__note{display:block;margin-top:5px;color:var(--cc-muted);font-size:13.5px;line-height:1.48;letter-spacing:0;overflow-wrap:anywhere}.cc-aff-card__cta{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;background:#111827;color:#fff;font-weight:720;font-size:13.5px;line-height:1;padding:11px 16px;white-space:nowrap}.cc-aff-card__cta:after{content:\"↗\";margin-left:7px;font-size:12px}.cc-aff-card--oshc,.cc-aff-card--ovhc,.cc-aff-card--studenthealth{--cc-accent:#0f766e}.cc-aff-card--flywire{--cc-accent:#1d4ed8}.cc-aff-card--sleek{--cc-accent:#7c3aed}.cc-aff-card--airalo{--cc-accent:#0891b2}.cc-aff-card--kkday,.cc-aff-card--shopping{--cc-accent:#db2777}.cc-aff-card--bizcover{--cc-accent:#b45309}.cc-aff-card--dataforseo{--cc-accent:#0f766e}@media(max-width:560px){.cc-aff-card{grid-template-columns:auto 1fr;margin:24px 0;padding:16px;gap:12px}.cc-aff-card__cta{grid-column:1/-1;width:100%;padding:12px 14px}.cc-aff-card__mark{width:40px;height:40px;border-radius:12px}.cc-aff-card__title{font-size:16px}}.cc-aff-stack{margin:30px 0 8px}.cc-aff-stack .cc-aff-card{margin:0 0 12px}.cc-aff-stack__note{margin:0;font-size:12px;line-height:1.6;color:#6b7280}</style>
<!-- AFF-CARD:v1:END -->
