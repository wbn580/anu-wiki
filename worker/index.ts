// anu-wiki —— 静态资产 + D1 运行时文章层（R254 通用模板实例）
//
...
function composeSitemap(base: string, rows: ArticleIndexRow[]): string {
  ...
  const indexClose = base.lastIndexOf("</sitemapindex>");
  if (indexClose < 0 || base.includes(D1_SITEMAP_PATH)) return base;
  const entry = `<sitemap><loc>${escapeXml(ORIGIN + D1_SITEMAP_PATH)}</loc></sitemap>`;
  return base.slice(0, indexClose) + entry + "\n" + base.slice(indexClose);
}

function composeD1Sitemap(rows: ArticleIndexRow[]): string {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // 索引路由必须先于静态资产返回；静态重定向也不能盖掉 D1 实时索引。
    // sitemapindex 正本通过独立的 D1 子地图保持 XML 类型合法。
    if (url.pathname === D1_SITEMAP_PATH) {
      try {
        return new Response(composeD1Sitemap(await listArticles(env)), {
          headers: {
    ...
    // 普通静态资产命中就直接返回；只有 404 才回落 D1 文章。
