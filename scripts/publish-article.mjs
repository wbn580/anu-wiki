...
const transient = /fetch failed|econnreset|etimedout|socket|network|timed?\s*out|incomplete\s*read|rate.?limit|internal server|\b5\d\d\b/i;
for (let attempt = 1; attempt <= 3; attempt += 1) {
  try {
    const output = execFileSync(cmd, wranglerArgs, {
      encoding: "utf8",
    ...
    if (attempt === 3 || !transient.test(`${stdout}\n${stderr}\n${error?.message || ""}`)) {
      throw error;
    }
    console.error(`D1 写入遇到瞬时外部错误，2 秒后重试（${attempt + 1}/3）...`);
